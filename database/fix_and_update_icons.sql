...
-- 第一步：修改icon字段长度
-- ============================================
-- 将icon字段从varchar(10)改为varchar(255)，以便存储图片路径
...
WHERE table_name = 'pet_breeds' AND column_name = 'icon';

-- 查询所有使用图片路径的品种（应该看到所有84个品种）
SELECT COUNT(*) as total_with_images
FROM pet_breeds 
WHERE icon LIKE '/images/pets/%';

-- 查询所有仍使用emoji的品种（应该为0）
SELECT COUNT(*) as total_with_emoji
FROM pet_breeds 
